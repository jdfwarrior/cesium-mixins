...
import type { UiConfig, UiIcon, UiPosition } from "../types";
...
export class Helper {
  ...
  constructor(viewer: Viewer, config?: UiConfig) {
  ...
  show() {
    try {
      if (!this.ele) return;
      this.viewer?.canvas.parentElement?.appendChild(this.ele);
      this.visible = true;
    } catch {}
  }
  ...
  hide() {
    try {
      if (!this.ele) return;
      this.viewer?.canvas.parentElement?.removeChild(this.ele);
      this.visible = false;
    } catch {}
  }
}
